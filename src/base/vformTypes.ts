...
export namespace VForm {
  ...
  export type TFormValuesByName<T, E> = Record<string, TFormValue<T, E>>;
   ...
   *  @typeParam E 欄位次要 payload 型別，用於延伸擴展，可以是 {}
   **/
  export type TFormState<T, E> = Record<TFormKey<T, E>, TFormField<T, E>>;

  export type TFormPayload<T, E> = Record<TFormKey<T, E>, TFormValue<T, E>>;
   ...
   *  ```
   * */
  export type TValidationRuleHandler = (
    ctx: IBaseFormContext<any, any>,
    ...args: any[]
  ) => boolean;
  export type TValidationRules<K extends string> = Record<
    K,
    (ctx: IBaseFormContext<any, any>, ...args: any[]) => boolean
  >;

  export type TValidationMessages = Record<string, ComputedRef<string>>;
