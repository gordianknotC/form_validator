...
export declare namespace VForm {
     ...
     *  @typeParam E 欄位次要 payload 型別，用於延伸擴展，可以是空物件
     **/
    type FormState<T, E, V> = Record<FormKey<T, E, V>, FormField<T, E, V>>;
    type FormPayload<T, E, V> = Record<FormKey<T, E, V>, FormValue<T, E, V>>;
    /**
     ...
     * @typeParam V - object containing keys of all validators
     */
    type InternalValidators<V, F = any> = Record<keyof V, InternalValidator<V, F>>;
    /** 用來定義驗證規則所對應的驗證訊息
     * {@link ValidationMessages}
     * @typeParam V - validators
     */
    type UDValidationMessages<V> = Record<keyof (V), Optional<ComputedRef<string>>>;
    /** 用來定義驗證規則所對應的驗證訊息
     * 鍵為欄位名，值必須為 {@link ComputedRef}，用來追踪i18n狀態上的變化
     * @typeParam V - validators
     */
    type ValidationMessages<V> = Record<keyof (V), ComputedRef<string>>;
    /**
     * @typeParam V - object containing keys of all validators
     * */
    type UDValidatorLinkHandler<V = string> = (linkField?: string) => string;
    type UDValidator<V, F = any> = {
        handler: ValidatorHandler<V, F>;
    };
    /**
     * @typeParam V - object containing keys of all validators
     */
    type UDValidators<V> = Record<keyof V, UDValidator<V>>;
    /**
     * @typeParam V - validators
     * @typeParam R - 使用者自定義 rules {@link UDFieldRules}
     */
    type UDFieldRuleConfig<R, V> = {
        ident: keyof R;
    ...
    * ```
    */
    type UDFieldConfigs<F, V> = Record<keyof F, VForm.FormField<F, F, V>>;
    /**
      * 使用者自定義欄位設定
      * @typeParam F - 所有欄位 payload 型別聯集
      * @typeParam V - validators
      ...
      * @see {@link defineFieldConfigs}
      */
    type UDFieldDefineMethod<F, V, R> = (option: Pick<VForm.FormField<F, F, V>, "placeholder" | "hidden" | "disabled" | "label" | "fieldType" | "payloadKey"> & {
        fieldName: string;
     ...
     * */
    abstract class IBaseFormContext<T, E, V> {
        /**
     ...
     * */
    abstract class IBaseFormModel<T, E, V> {
     ...
     *
     * */
    abstract class IBaseFormCtrl<T, E, V> {
     ...
     *
     * */
    abstract class IBaseEventHandler<T, E, V> {
